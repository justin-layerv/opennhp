...
- **`InternalAuthSignerUnavailable`** non-zero during an operator-triggered sweep — the server is reachable but lacks `NHP_INTERNAL_AUTH_SECRET`, so the signed fast path returns 401. Treat this as deployment/config drift and alert the platform owner before relying on the endpoint during an incident. The "alert the platform owner" step is **automated**: a Terraform-managed CloudWatch alarm (`<name_prefix>-<cell>-internal-auth-signer-unavailable`, `terraform/modules/monitoring/main.tf`, #2475) pages the alerts SNS topic on any non-zero value in the trailing hour. It is **reactive** — it fires only once an operator hits the endpoint with the secret already missing, so it cannot surface the drift ahead of an incident; a proactive detector would need a synthetic signed probe.
...
| `InternalAuthSignerUnavailable` non-zero during a sweep attempt | Missing or misrotated `NHP_INTERNAL_AUTH_SECRET` on the target server | Paging the platform owner is automated via the `internal-auth-signer-unavailable` CloudWatch alarm (reactive — fires only once the endpoint is hit). Redeploy/fix the secret before depending on the on-demand sweep. Background sweeps still enforce revocations when F5 strict mode is on. |
