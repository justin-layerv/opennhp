...
`NHP_RACK` or ages out to `RevocationAgedOut`.

- [ ] Pre-rollout: confirm the deployed AC image includes the `NHP_RACK` ack path (`RevocationAckSent` exists in AC metrics/logs, or the image tag is at/after this code) before refreshing servers. A pre-ACK AC fleet will correctly age out every revoke.
