...
- [ ] Post-apply: confirm a server blue/green deploy still works with the public UDP
      listener gone — `blue-green-switch.sh` skips the UDP flip **only because the
      `/<env>/nhp/server/take-server-private` marker is `true`** (a missing listener
      without the marker hard-fails, guarding botched public deploys), and still records
      active-color (covered by `tests/scripts/blue-green-switch_test.sh`; verify a real
      dispatch).
...
- [ ] Post-apply: confirm `internal_tg_no_healthy_targets` is the live server-health
      alarm (the public-NLB `unhealthy_hosts`/`no_healthy_hosts`/`tcp_resets` alarms
      are now gated off in sandbox).
...
- [ ] **Reduced canary fence:** when private, the server canary sets
      ...
      **CPU + ASG-instance health only** — there is no per-color NLB health fence during
      the rollout (the internal NLB fronts both colors via a static attach, so it can't
      gate per-color). Confirm CPU/ASG checkpoints are tuned tightly enough before the
      first private prod canary, or accept the reduced fence explicitly.
