...
- [ ] Rollback: revert the three roots to `["3.141.109.76/32"]` and re-apply. The
      module validation still accepts an exact `/32` list, so rollback needs no
      module change. Note this re-breaks developer access by design and should
      supersede ADR 0001 rather than be done silently.
