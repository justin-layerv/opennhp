...
- [ ] Rollback: **do not use the address this entry originally named.** It said
      ...
      runner's EIP, and the runner was destroyed on 2026-08-10 (#3804), so the
      address was released back to the AWS public pool and may already belong to
      another account. Verified 2026-08-10 that no live security-group rule and
      no committed root still carries it.

      **First step of any re-fence: prove you own the address.**
      `aws ec2 describe-addresses --filters Name=public-ip,Values=<addr>` must
      return it under this account. That check is the actual control here and it
      generalises — ownership cannot be expressed in a Terraform validation, so
      no guard downstream will catch a stale or foreign `/32` for you.

      Then allocate a **new EIP we own** and set the three roots to that `/32`.
      `modules/compute` accepts `null`, exactly `["0.0.0.0/0"]`, or a list of
      exact `/32`s, so no module change is needed. The NLBs already carry
      security groups (#3453 converged), so a re-fence changes sources only — it
      does not replace load balancers. It re-breaks developer access by design
      and should supersede ADR 0001 rather than be done silently.

      Note what does and does not guard this. All three roots are
      validation-pinned — prod to `null`, cell0 and cell1 to exactly
      `["0.0.0.0/0"]` — so following the original instruction verbatim fails at
      plan time rather than silently allowlisting a stranger. But those guards
      protect the *openness decision*, not address ownership: a legitimate
      re-fence edits them by design, and past that point nothing stops a stale
      address being used. `.github/scripts/check-relay-dmz-plan.py` still
      accepts the retired `/32`; retiring that allowance is tracked in
      [`2026-07-26-pr-3453-udp-source-fence.md`](2026-07-26-pr-3453-udp-source-fence.md).
