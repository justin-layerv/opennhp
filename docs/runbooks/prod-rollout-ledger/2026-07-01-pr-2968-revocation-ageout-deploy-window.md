...
- [ ] Security acceptance: confirm this PR's bounded interim tradeoff does not block prod arming. A single raw `RevocationAgedOut` inside the recent deploy-heartbeat window can be suppressed from paging rather than delayed, while the raw alarm and `revocation-aged-out-suppressed` breadcrumb remain visible for audit. This is accepted for the issue #2868 deploy-noise fix only while [#2974](https://github.com/layervai/nhp/issues/2974) tracks hardening against suppressors without real deploy evidence.
...
- [ ] Post-rollout: confirm deploy workflows can emit `DeploymentWindow` in prod by checking a promote/blue-green/canary run log for the `DeploymentWindow metric pushed` breadcrumb, or by observing a fresh `LayerV/NHP` `DeploymentWindow` datapoint with dimensions `{Environment=prod, Cell=<cell>}`.
...
- [ ] Follow-up: track [#2974](https://github.com/layervai/nhp/issues/2974) until `DeploymentWindow` cannot be silently emitted by non-deploy principals or has a watchdog for windows without a corresponding deploy signal.
