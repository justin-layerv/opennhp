...
- [x] Pre-rollout: confirm the scanner image-tag SSM parameter points at a qurl-service SHA that includes `QURL_SCANNER_SCAN_MODE=active-resource-recheck`, root event `scan_mode` support, and the real-run guard requiring tombstone-write for active rechecks. _Done/current 2026-06-17: SSM points at qurl-service `cef5f89`; source at that SHA includes `active-resource-recheck`, root-level `scan_mode`, and the `--scan-mode="active-resource-recheck" requires --enable-tombstone-write` real-run guard._
- [x] Pre-rollout: confirm reserved-concurrency headroom before prod/non-trivial-cell enablement; the scanner pair reserves two Lambda concurrency slots per qurl-service cell once active recheck is enabled. _Done/current 2026-06-17: `aws lambda get-account-settings` reported unreserved concurrency 995 in sandbox and 997 in prod._
