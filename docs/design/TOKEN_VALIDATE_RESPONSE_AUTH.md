...
After request HMAC verification succeeds and the nonce is valid, downstream validation and store errors are signed too. This includes invalid JSON, missing token, overlong `agent_run_id`, invalid token responses, expired token responses, `run_id_mismatch` responses, and token-store unavailability. Signing those bodies prevents an internal path-positioned tamper point from forging the error payload.

For a live token entry with a non-empty stored RunID, the request must supply the same non-empty value. A different or omitted `agent_run_id` returns signed HTTP 200 with `valid=false`, `error=run_id_mismatch`, and `run_id` echoing only the request (therefore absent on omission); stored source, user, owner, and expiry metadata remain omitted. Every `valid=true` response similarly returns `run_id` only as an empty-or-equal echo of the request, never as an unsolicited stored identifier.

This fail-closed distinction reveals one bit to an authenticated internal caller: omitting `agent_run_id` distinguishes a live entry with a RunID binding (`run_id_mismatch`) from a current unbound entry (`valid=true`). That is deliberate and does not reveal the stored value. The caller already holds the shared internal-auth secret; this endpoint must remain inside that authenticated trust boundary.

`agent_run_id` remains optional only for current entries whose stored RunID is empty. That preserves the current producer shape while making omission fail closed as soon as a producer stores a binding. Producer activation is tracked in [NHP #3226](https://github.com/layervai/nhp/issues/3226): because ACK minting precedes FRP Login, qurl-go supplies the canonical 16-lowercase-hex generator/validator, qURL Connector calls it once per outer cycle and owns reuse, qurl-go carries the explicit value in the authenticated KNK body, and the connector seeds that same value into the first FRP Login through a supported FRP client API. `Knock` must not generate implicitly, and `NewACKTokenEntry` cannot derive a later server-generated RunID from registration. The final qurl-reverse-tunnel-server consumer contract must require that supplied non-empty RunID, assert it on every validation request, and reject the Login before validation if it is missing or noncanonical; retaining a server-autogenerated accepted path would make the ACK-time binding structurally impossible.
