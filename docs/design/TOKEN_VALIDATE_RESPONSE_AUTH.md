...
`agent_run_id` remains optional only for intentional legacy/HTTP entries and pre-producer rows whose stored RunID is empty. Registered-agent native UDP ACKs now store the canonical binding from the authenticated KNK application body, so omission fails closed for that path. Because ACK minting precedes FRP Login, qurl-go supplies the canonical 16-lowercase-hex generator/validator, qURL Connector calls it once per outer cycle and owns reuse, qurl-go carries the explicit value in the authenticated KNK body, and the connector seeds that same value into the first FRP Login through a supported FRP client API. `Knock` must not generate implicitly, and `NewACKTokenEntry` cannot derive a later server-generated RunID from registration. The final qurl-reverse-tunnel-server consumer contract must require that supplied non-empty RunID, assert it on every validation request, and reject the Login before validation if it is missing or noncanonical; retaining a server-autogenerated accepted path would make the ACK-time binding structurally impossible.
