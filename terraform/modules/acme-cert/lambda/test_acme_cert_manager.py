...
class TestAcmeStackCompat:
    ...
    'X509Req'" — while pip resolution and the fully-mocked tests above stayed
    green (none call the real new_order()). acme>=3 / josepy>=2 parse CSRs with
    `cryptography` instead.

    This drives a real CSR through the pinned acme's new_order() with only the
    HTTP transport mocked, so a future dependency bump that reintroduces the
    removed pyOpenSSL APIs fails here instead of silently in production.
    ...
    def test_new_order_parses_csr_without_pyopenssl_x509req(self):
        acm.lazy_import_acme()  # exercise the Lambda's own import path
        ...
        assert not hasattr(acm.josepy, 'ComparableX509')
        ...
        acme_client = client.ClientV2(
            ...
            net=client.ClientNetwork(acm.josepy.JWKRSA(key=key)),
        )
        ...
        with patch.object(acme_client, '_post', side_effect=_StopBeforeHTTP) as mock_post:
            with pytest.raises(_StopBeforeHTTP):
        ...
        assert domain in [ident.value for ident in order.identifiers]
    ...
    def test_handler_acme_surface_resolves(self):
        ...
        for obj, attr, sig_check in [
            ...
            (acm.josepy, 'JWKRSA', False),
        ]:
            assert hasattr(obj, attr), f'{obj!r} no longer has {attr}'
            member = getattr(obj, attr)
            assert callable(member), f'{attr} is not callable'
            if sig_check:
