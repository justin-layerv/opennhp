...
#
# Two complementary alarms cover the revocation-latency rollout gate: the latency
# p99 alarm catches SLOW-but-delivered revokes; the aged-out alarm catches
# NEVER-delivered ones (neither alone proves delivery). The full rationale —
# the SLO number, the EMF-backs-p99 detail, the histogram coverage boundary — is
# canonical on RevocationDeliveryLatencyP99SLO in endpoints/server/revocation_retry.go.
#
# Both key on the nhp-server publisher's base dim set {Environment, Cell}; keep
# in lockstep with buildServerMetricDimensions() in endpoints/server/udpserver.go
# (terraform/CLAUDE.md "Metric / Alarm Dim-Set Rules" — a partial dim set selects
# a non-existent stream and the alarm sits in INSUFFICIENT_DATA forever).
#
# Both use treat_missing_data = "notBreaching": the revocation retry engine is
# default-OFF (NHP_REVOCATION_RETRY_ENABLED), so until it is armed fleet-wide
# there are no samples and no aged-out events. Absence is "no revokes / engine
# not yet armed," not a fault — matches the KnockLatency precedent.
