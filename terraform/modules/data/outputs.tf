...
output "etcd_ca_cert_arn" {
  ...
  value       = local.deploy_etcd ? aws_secretsmanager_secret.etcd_tls[0].arn : null
}

output "etcd_secret_arn" {
  description = "etcd credentials secret ARN"
  value       = local.deploy_etcd ? aws_secretsmanager_secret.etcd[0].arn : null
}
...
output "etcd_security_group_id" {
  ...
  value       = local.deploy_etcd ? aws_security_group.etcd[0].id : null
}
