...
# ============================================================================
# Create resource.toml - maps Auth Service Provider ID to plugin paths
# This tells the NHP Server which plugin handles which aspId
# The server reads this from etc/resource.toml (see config.go:loadResources)
# ============================================================================
%{ if length(server_plugins) > 0 ~}
...
# Maps aspId to plugin paths for HTTP authentication
...
%{ endfor ~}
RESEOF
echo "resource.toml created for plugins: ${join(", ", keys(server_plugins))}"
%{ else ~}
