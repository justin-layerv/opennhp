...
# ============================================================================
# FRPS bootstrap overlay (Wave 5 prep — SLACK_QURL_ROLLOUT.md §6).
#
# Until issue #1976 (DDB→authServiceMap bridge) lands, the rows that
# `terraform/resources.tf` writes to the `nhp_resources` table are NOT
# read at runtime — `endpoints/server/config.go::loadResources` only
# reads this file. Appended here so the FRPS resource group reaches the
# authServiceMap on boot.
#
# The overlay is rendered by Terraform (`local.frps_resource_toml_overlay`
...
# server_plugins is empty the plugin section is skipped and this becomes
...
#
#   - `length(server_plugins) > 0` (today's production path): the
#     plugin block above uses `cat > resource.toml` which truncates the
#     file on every re-exec, so the overlay append below ALWAYS runs
#     against a fresh file. The sed-strip below is a no-op on this
#     path (no prior overlay to strip). Net behavior: every re-exec
#     ends with the same plugin-block-then-overlay shape.
#
...
#
# The sentinel is a version-stamped comment paired with a unique
# end-sentinel. Both literals are threaded in from Terraform
# (`local.frps_overlay_sentinel_prefix` and
# `local.frps_overlay_end_sentinel` in terraform/resources.tf), so the
# grep + sed patterns below cannot desync from the overlay body — a
# future rename of the sentinel flows through both producer (overlay
# render) and consumer (idempotency strip) automatically.
#
# The sed-strip runs unconditionally before each append: it deletes
# the line range `[start_sentinel..end_sentinel]` in `resource.toml`.
# Self-heals across sentinel version bumps (v1 → v2 → v3 …) — a host
# carrying the v1 block on a user_data re-exec gets v1 stripped and
# v2 appended in the same run. The end-sentinel is necessary because
# `Addr.Protocol` repeats per resource group; with multi-group
# overlays a `,/^Addr\.Protocol/d` range would terminate at the first
# group's `Addr.Protocol` and leave the rest of the overlay in place.
#
# The sed pattern uses `[0-9]\+` (BRE) on the start anchor so it's
# not pinned to the current version; the end anchor is the literal
# end-sentinel string, which appears exactly once per overlay
# regardless of resource-group count.
# ============================================================================
%{ if frps_resource_toml_overlay != "" ~}
RESOURCE_TOML=/opt/layerv/nhp-server/etc/resource.toml
touch "$RESOURCE_TOML"
chmod 644 "$RESOURCE_TOML"
# Self-healing strip: remove any prior overlay block (sentinel line
# through end-sentinel line) before appending. No-op on fresh files /
# the with-plugins truncate path. The grep + sed patterns share the
# same TF-threaded `frps_overlay_sentinel_prefix` literal AND match the
# same lines: any line starting with the literal prefix. Dropping a
# `[0-9]\+` digit-suffix requirement on the sed range head matches the
# grep existence check exactly, so a sentinel without a trailing digit
# (operator hand-edit, future version-less rename) can't pass grep but
# fail sed and silently produce duplicate tables in `resource.toml`
# (pelletier/go-toml/v2 errors on duplicate tables → loadResources
# log-and-skips the entire file → fleet-wide resource-map loss).
if grep -q "^${frps_overlay_sentinel_prefix}" "$RESOURCE_TOML"; then
  echo "Stripping prior FRPS bootstrap overlay from resource.toml (user_data re-exec or sentinel bump)..."
  sed -i "/^${frps_overlay_sentinel_prefix}/,/^${frps_overlay_end_sentinel}/d" "$RESOURCE_TOML"
fi
...
# delimiter literal can't accidentally terminate the heredoc early.
# The rendered TOML body comes from Terraform-fenced inputs that
# can't include this string today, but the long delimiter is a
# cheap one-line defense.
cat >> "$RESOURCE_TOML" << 'OVERLAYEOF_FRPS_V2_DO_NOT_EDIT'
${frps_resource_toml_overlay}
OVERLAYEOF_FRPS_V2_DO_NOT_EDIT
echo "FRPS bootstrap overlay applied to resource.toml"
%{ endif ~}
