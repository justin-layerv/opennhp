...
# (relay_enabled), the server trusts the relay fleet's pubkey so it accepts the
# NHP_RLY forwards a browser->relay knock produces. The relay fleet shares ONE
# keypair; boot-read its PUBLIC key from Secrets Manager (the secret is rendered
# by modules/relay; the server reads it by the deterministic name to avoid a
# compute<->relay module cycle). Paired with DisableRelayValidation=true in
# config.toml above — both gated on relay_enabled, so prod (deploy_relay=false)
# stays behaviorally dark. NOT fatal if the secret is missing: the relay path is
# inert until the key exists. Boot-only: relay.toml is written once here. The
# server watches it (relayConfigWatch in config.go), but user_data never rewrites
# it, so enabling the relay on an already-running fleet needs an instance refresh
# (or the secret present at launch) — there is no live pickup of a secret that is
# populated later. The same applies to key ROTATION: a rotated fleet keypair is
# not adopted until the server fleet is refreshed, so the server would reject the
# new relay's NHP_RLY until then (the keygen lambda generates once, so this is
# latent today, not imminent). Live pickup / rotation without a refresh is
# subsumed by the dynamic relay registry that replaces static relay.toml (#2541).
# ============================================================================
%{ if relay_enabled ~}
echo "Fetching relay fleet public key from Secrets Manager (${relay_secret_name})..."
# SECURITY: the relay secret JSON carries the fleet PRIVATE key (not just the
# public key we use). Bracket fetch + extract with set +x so xtrace doesn't echo
# the private key into user-data.log -> CloudWatch; scrub the JSON after. Mirrors
# the server/etcd/cookie secret fetches above. RELAY_PUBKEY itself is public.
set +x
RELAY_SECRET=$(aws secretsmanager get-secret-value --secret-id "${relay_secret_name}" --region "$REGION" --query SecretString --output text 2>/dev/null) || RELAY_SECRET=""
RELAY_PUBKEY=""
if [ -n "$RELAY_SECRET" ]; then
  RELAY_PUBKEY=$(echo "$RELAY_SECRET" | jq -er ".publicKey") || RELAY_PUBKEY=""
fi
unset RELAY_SECRET
set -x
if [ -n "$RELAY_PUBKEY" ]; then
  # Unlike config.toml (chmod 600 — carries the fleet PRIVATE key), relay.toml
  # holds only the relay's PUBLIC key + placeholder Ip/Port, so it is left at the
  # default mode; there is nothing secret to protect.
...
RELAYEOF
  echo "relay.toml written; server now trusts the relay fleet pubkey"
else
  # Runtime miss (IAM lag, transient Secrets Manager error, or secret not yet
  # populated): non-fatal so the server still boots to serve direct knocks, but
  # it rejects every NHP_RLY until the key exists. A CloudWatch alarm for this
  # server-side "relay enabled but forwards rejected" signal is tracked in #2630.
  echo "WARNING: relay_enabled but relay pubkey unavailable (secret ${relay_secret_name} missing/unreadable or no publicKey); relay.toml NOT written — server will reject relay forwards until the key exists. Not fatal: the relay path is dark until then."
fi
%{ endif ~}
