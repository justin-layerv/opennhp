...
%{ if qurl_router_enabled ~}
# Custom domain routers are added dynamically via custom-domains.toml
# (generated by custom-domain-cert-sync.sh). Each custom domain gets an
# explicit Host() router at priority 20 with its own TLS cert.
# No catch-all needed — domains without a provisioned cert should not
# serve traffic, and the nhp-ac fallback router handles unknown hosts.
echo "Custom domain routing enabled (routers via custom-domains.toml)"
%{ endif ~}
...
# Fetches all certs from Secrets Manager and creates custom-domains.toml.
# Traefik file watcher will pick up the config changes automatically.
# ============================================================================
echo "Syncing custom domain certificates..."
export SECRETS_PREFIX="custom-domain-cert"
