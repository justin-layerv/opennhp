...
resource "aws_cloudwatch_metric_alarm" "aop_replay_detected" {
  ...
  statistic           = "Sum"
  threshold           = 0
  alarm_description   = "AC dropped replayed/duplicate AOP packets within the replay cache TTL. A single isolated event can follow restart/failover retries; sustained or repeated ALARM transitions indicate replay attempts or a broken retry path."
  treat_missing_data  = "notBreaching"
