# Login Portal
#
# Hosts the login portal static page for sign-in (Auth0) and access code redemption.
# S3+CloudFront static site with OAC, cloned from qurl-link module pattern.
...
locals {
  # Generate the login portal HTML with configured Auth0 and API URLs
  index_html = <<-HTML
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .sign-in-btn:hover { background: #00b8d9; }
    ...
    .code-input:focus { border-color: #00d4ff; }
    ...
    .submit-btn:hover { background: #00b8d9; }
...
<body>
  <div class="container">
    ...
    <div id="panel-code" class="panel">
      <p class="subtitle">Enter an access code to view shared resources.</p>
      <form id="code-form" autocomplete="off">
        <input type="text" class="code-input" id="code-input" placeholder="ac_..." maxlength="25" pattern="ac_[a-z0-9_\-]{22}" spellcheck="false" autocomplete="off" aria-label="Access code" required>
        <div class="hp-field" aria-hidden="true"><input type="text" id="hp-field" name="website" tabindex="-1" autocomplete="off"></div>
        <button type="submit" class="submit-btn" id="submit-btn">
          <span id="submit-text">Redeem Code</span>
    ...
    <div class="footer"><a href="https://layerv.ai" target="_blank" rel="noopener noreferrer">Powered by LayerV</a></div>
  </div>

  <script>
    (function() {
      'use strict';
      var API_URL = '${var.qurl_api_url}';
      var AUTH0_DOMAIN = '${var.auth0_domain}';
      var AUTH0_CLIENT_ID = '${var.auth0_client_id}';
      var AUTH0_AUDIENCE = '${var.auth0_audience}';
      var AUTH0_REDIRECT_URI = '${var.auth0_redirect_uri}';
      ...
      // Deep link: ?tab=code selects Access Code tab
      ...
      var hpField = document.getElementById('hp-field');
      ...
      form.addEventListener('submit', function(e) {
        ...
        fetch(API_URL + '/v1/access-codes/redeem', {
          method: 'POST',
        ...
        .then(function(result) {
          if (result.ok && result.data.data && result.data.data.redirect_url) {
            window.location.href = result.data.data.redirect_url;
          } else {
            var detail = (result.data.error && result.data.error.detail) || 'Invalid access code. Please check and try again.';
        ...
        .catch(function(err) {
          console.error('Access code redeem failed:', err);
      ...
      function resetButton() {
        ...
        submitText.textContent = 'Redeem Code';
...
# S3 bucket for the login portal page
resource "aws_s3_bucket" "login_portal" {
...
resource "aws_s3_bucket_versioning" "login_portal" {
  bucket = aws_s3_bucket.login_portal.id
  versioning_configuration {
    status = "Enabled"
  }
}

resource "aws_s3_bucket_server_side_encryption_configuration" "login_portal" {
  bucket = aws_s3_bucket.login_portal.id
...
resource "aws_s3_bucket_public_access_block" "login_portal" {
  bucket = aws_s3_bucket.login_portal.id
...
resource "aws_cloudfront_origin_access_control" "login_portal" {
...
resource "aws_s3_bucket_policy" "login_portal" {
  bucket = aws_s3_bucket.login_portal.id
    ...
    Statement = [
      {
        ...
        Action   = "s3:GetObject"
        Resource = "${aws_s3_bucket.login_portal.arn}/*"
        Condition = {
          StringEquals = {
            "AWS:SourceArn" = aws_cloudfront_distribution.login_portal.arn
          }
        }
      }
    ]
...
resource "aws_cloudfront_response_headers_policy" "login_portal" {
  name = replace("${var.domain_name}-security-headers", ".", "-")

  security_headers_config {
    content_security_policy {
      content_security_policy = "default-src 'self'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src ${var.qurl_api_url} https://${var.auth0_domain}"
      override                = true
    }
...
resource "aws_cloudfront_distribution" "login_portal" {
  enabled             = true
  is_ipv6_enabled     = true
  default_root_object = "index.html"
  aliases             = [var.domain_name]
  price_class         = "PriceClass_100" # US, Canada, Europe
  comment             = "Login Portal - ${var.domain_name}"

  origin {
    domain_name              = aws_s3_bucket.login_portal.bucket_regional_domain_name
    origin_id                = "S3-${var.bucket_name}"
    origin_access_control_id = aws_cloudfront_origin_access_control.login_portal.id
  }
  ...
  default_cache_behavior {
    ...
    response_headers_policy_id = aws_cloudfront_response_headers_policy.login_portal.id
  ...
  # Return index.html for all paths (handles SPA routing)
...
resource "aws_s3_object" "index" {
  ...
  tags = merge(var.tags, { Component = "login-portal" })
}
