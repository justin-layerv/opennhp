...
variable "enable_l3_flush_on_expiry" {
  description = "Prod AC L3 flush-on-expiry scheduler: actively tear down kernel allow-state (ipset/BPF map + conntrack) at session end. Off by default (pre-flush baseline). Full semantics in modules/ac/variables.tf, rollout in docs/runbooks/l3-flush-*.md."
  type        = bool
  default     = false
}

variable "l3_flush_dry_run" {
  description = "Gate the prod L3 flush scheduler into log-only mode (default true); no effect unless enable_l3_flush_on_expiry=true. Full semantics + the first-load dry-run safety in modules/ac/variables.tf."
  type        = bool
  default     = true
}
...
variable "l3_flush_conntrack_pool_size" {
  description = "Prod AC netlink conntrack socket pool size. 0 preserves the AC default (currently 16)."
  type        = number
  default     = 0
...
variable "nhp_revocation_retry_enabled" {
  description = "Wrapper passthrough for the root nhp_revocation_retry_enabled (#2793). Default false keeps new/unset environments conservative; prod opts in explicitly in terraform.tfvars after ACK support."
  type        = bool
  default     = false
}
...
variable "qurl_link_js_agent_enabled" {
  description = "Upload the browser NHP JS-agent bundle to qurl.link, render relay bootstrap config, and disable browser resolve ingress. Requires deploy_relay and relay_dns_name when true. Prod remains false until the sandbox relay browser cutover is proven and a dedicated prod cutover PR flips it."
  type        = bool
  default     = false
}
...
variable "relay_vpc_cidr" {
  description = "Dedicated relay DMZ VPC CIDR. Ignored while deploy_relay=false."
  type        = string
...
# the sandbox wrapper, and modules/compute/variables.tf.
