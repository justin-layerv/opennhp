...
#   * modules/udp-proof-runner grants the runner role DescribeKey + Decrypt on
...
# the module additionally constrains the runner's decrypt to the
# qurl-agent-x25519-private-key encryption context.
# -----------------------------------------------------------------------------

data "aws_caller_identity" "current" {}

resource "aws_kms_key" "proof_agent_seal" {
  description             = "Sandbox UDP-proof: seals the Connector proof agent's x25519 private key"
  deletion_window_in_days = 7
  enable_key_rotation     = true

  # Default-shape policy: account root administers; principals get key use via
  # their IAM policies (the runner's scoped Decrypt grant lives in
  # modules/udp-proof-runner/iam.tf, the agent's Encrypt in the attended setup).
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
