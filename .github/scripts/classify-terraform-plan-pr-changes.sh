...
while IFS=$'\t' read -r status file _rest; do
	...
	case "$file" in
		...
		terraform/environments/prod/*)
			...
			;;
		terraform/*|.github/workflows/terraform-plan-pr.yml|.github/actions/build-lambda-packages/*|.github/scripts/classify-terraform-plan-pr-changes.sh|.github/scripts/fetch-auth0-token.sh)
			terraform_changed=true
