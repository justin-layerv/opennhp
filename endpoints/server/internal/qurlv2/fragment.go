...
// the scheme/host (pass the fragment only).
...
// use. See layervai/qurl-service#1002 and validateClaimValues in parse.go.
...
// signed bytes and the transmitted bytes are identical (the issuer signs the
// base64url string it then embeds). It returns the body WITHOUT a leading "#";
// callers prepend "#"/the URL.
//
...
// a stray "." (the field separator — not in the base64url alphabet, so a dotted
// part would otherwise split into the wrong number of fields and produce an
// unparseable fragment), padding, and non-canonical encodings. With the signature
// validated to the pinned 64-byte low-S form, BuildFragment can never emit a body
// that ParseFragment would reject — round-trip safety is guaranteed at the source.
func BuildFragment(claimsB64, secretB64 string, rawSig []byte) (string, error) {
	...
	// so it reads as a fragment-assembly contract violation while preserving the
	// underlying ErrEncoding for callers that match on it.
