...
// vectorFilePath is the committed golden-vector fixture. It is structured as a
// standalone JSON document so the js-agent (later slice) can consume the exact
// same file for WebCrypto-parity verification.
const vectorFilePath = "testdata/issuer_signature_vectors.json"

// TestGoldenVectors_Consume is the always-run contract test. It loads the
// committed fixture and asserts each vector accepts or rejects exactly as the
// fixture declares, with the precise sentinel for each rejection class. It FAILS
// (never skips) if the fixture is missing/unparseable, so the contract can never
// silently drop out of CI.
func TestGoldenVectors_Consume(t *testing.T) {
	vf, err := LoadVectorFile(vectorFilePath)
	if err != nil {
		// FAIL, never skip: this fixture is the cross-language signature contract.
		// It is generated by qurl-service (the issuer/KMS-sign side) and copied
		// here verbatim; nhp only consumes it. Re-copy from
		// qurl-service internal/qurlv2/testdata/issuer_signature_vectors.json if missing.
		t.Fatalf("golden-vector fixture must load (copy from qurl-service internal/qurlv2/testdata): %v", err)
	}
