...
func parseSecret(raw []byte) (*Secret, error) {
	...
	// same shape discipline the claims path applies to the public keys. A
	// wrong-length secret is rejected here in the crypto core, not deferred to the
	// downstream PoP path. The error keeps both ErrStrictParse (parser contract)
	// and the underlying ErrKeyLength/ErrEncoding so callers can match either.
...
// into Claims.{Iat,Nbf,Exp}. Do NOT add dec.UseNumber() here — it is redundant
// (we decode into typed structs, not json.Number) and would not change behavior.
...
// nbf<=exp ordering bounds, and per-field key lengths. Time-field structural rules
// (integer-only, no float/exponent/string) are already enforced because the fields
// are typed int64 and decoded by a decoder that rejects fractional/exponent numbers
// for integer targets. Liveness (exp/nbf vs now) is intentionally NOT checked here
// — it is admission's job (see the ordering-bound comment below).
func validateClaimValues(c *Claims) error {
	...
	// crypto core vouch for nonsensical bytes. They are deliberately the only
	// ordering bounds enforced here. LIVENESS — exp-vs-now, nbf-vs-now, and clock
	// skew — is the admission layer's job (P3c/P3d), NOT this library's, because
	// this package has no trusted clock and the design assigns expiry/liveness to
	// admission. We do not enforce nbf>=iat (a backdated nbf is harmless given the
	// nbf/exp-vs-now checks admission already owns). The admission-time liveness
	// enforcement deferred here is tracked in layervai/qurl-service#1002 (NOT #996,
	// which is about pre-verification fragment/per-field size bounds).
	if c.Iat > c.Exp {
	...
	// cell_id is deliberately the ONE present-string claim allowed to be empty. Every
	// other required string above rejects an empty value, but cell_id is the single
	// OPTIONAL claim, so absent and present-empty are treated as equivalent "no cell"
	// and both accepted. (SignClaims uses omitempty, so a freshly-minted claim omits
	// an unset cell_id rather than emitting "cell_id":""; the parser still tolerates a
	// present-empty cell_id for forward/cross-encoder compatibility.) We intentionally
	// do NOT add an empty-cell_id rejection here — see Claims.CellID and
	// TestParseClaims_CellIDOptional.
	return nil
}
