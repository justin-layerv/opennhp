...
func CheckLiveness(c *Claims, now time.Time, skew time.Duration) error {
	...
	// Clamp to a sane upper bound so the int64 second arithmetic below cannot
	// overflow for a pathological caller. Real callers pass a small allowance
	// (v2ClockSkewAllowance = 60s).
	if skew > 24*time.Hour {
		skew = 24 * time.Hour
	}
	nowUnix := now.Unix()
	// Round (not truncate) to whole seconds so a sub-second allowance is not
	// silently dropped to zero.
	skewSecs := int64((skew + time.Second/2) / time.Second)
