// Package qurlv2 implements the qURL v2 wire format and issuer-signature crypto
// exactly per the design doc (nhp/docs/design/QURL_V2_KEYED_IDENTITY.md, sections
// "qURL v2 Artifact" and "Key Lifecycle → Issuer signing key").
//
// This is the security core: signed bootstrap claims, a strict allowlist parser,
// the issuer signing input + raw-r||s low-S signature handling, and a
// KMS-backed issuer signer. It is a self-contained library plus issuer-key infra,
// flag-gated, and NOT yet wired into issuance or admission (P3c/P3d/P3f).
//
...
const (
	...
	// blobs (empty, a raw 32-byte X25519 key, a multi-kilobyte RSA key). These
	// bounds match the resource-key range used elsewhere in qURL v2 so the
	// formats stay merge-compatible.
...
type Claims struct {
	...
	// since it is not required, and a present one). The signature is over the
	// received bytes either way, so this only affects newly-minted claims.
...
var (
	...
	ErrEncoding = errors.New("qurlv2: value is not valid unpadded base64url")
...
//
// Canonicality rests on the re-encode-and-compare below: it makes acceptance
// equivalent to "input == EncodeToString(decoded)", which covers BOTH non-canonical
// trailing bits AND embedded '\r'/'\n' (Go's base64 decoder silently skips those in
// every mode, including Strict — so without this check a newline injected into the
// unsigned secret part or the sig part would decode to the same bytes and the
// fragment would still verify, i.e. encoding malleability). Strict() on the decode
// is kept as a first-line filter that fails fast with a precise decoder error for
// the trailing-bits and bad-alphabet/padding cases; the re-encode check is the
// authoritative backstop for everything else.
...
// Strict() is a fast first-line filter only — the authoritative canonicality
...
// credential, so the crypto core rejects a wrong-length secret directly (the MD
// parser invariant: key fields are fixed-length base64url and must be decoded and
// length-checked before use) rather than deferring to the downstream PoP path.
...
// X25519 keys).
//
// Deliberate scope (v2 P3a): this performs ONLY a base64url-decode + length-window
// check on resource_public_key_b64. It intentionally does NOT parse the bytes as a
// real P-256 SPKI here, and that deferral has a known, accepted cost:
//   - On the verify path this is safe: the claims are issuer-signed, so a verifier
//     that has checked the signature is trusting the issuer to have minted a
//     structurally valid resource key; the field is not attacker-chosen there.
//   - The accepted cost is on the SIGN path: because the parse is skipped,
//     SignClaims -> parseClaims will sign a resource key that is length-window-valid
//     but structurally bogus (not a parseable P-256 SPKI). Running it through
//     ParseP256PublicKeyDER HERE would catch that at mint; we deliberately do not,
//     and instead defer the structural parse to the admission/resource-key consumer
//     (P3c/P3d), where the key is actually used for ECDH and where a parse failure
//     has request context to attribute and a metric to emit. The issuance-bug
//     surface that this deferral leaves open is tracked, not silently accepted.
//
// Tracking: layervai/qurl-service#1002 (admission-time resource_public_key P-256
// SPKI parse + liveness enforcement, deferred from this P3a library to the P3c/P3d
// consumers). See parse.go's iat/exp note for the matching liveness-deferral
// decision, which the same issue covers. (Distinct from #996, which is about
// pre-verification fragment/per-field SIZE bounds, not structural key validation.)
