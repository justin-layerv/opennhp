...
// signature. Verifiers (NHP servers and the first-party JS agent's Go counterpart)
// resolve the public key for a claim's kid from this small published store;
// unknown or retired kids are rejected. The store is deliberately KMS-independent
// so verification has no AWS dependency and the golden vectors verify offline.
//
// Rotation is overlap-publish and is expressed ENTIRELY through the contents of
// this map — there is no per-kid TTL, "retired" flag, or expiry field in the
// library. During overlap, the published map contains both the current kid and any
// recently-retired kids, so outstanding qURLs signed under an old kid keep
// verifying. "Retiring" a kid means the operator removes it from the published map
// (after the overlap window, once all qURLs it signed have expired); from that
// point publicKeyForKID returns ErrUnknownKID for it. The library never expires a
// kid on its own — a kid is accepted iff it is present in the map handed to
// NewTrustStore, and rejected the moment it is not.
...
func NewTrustStore(keys map[string]*ecdsa.PublicKey) (*TrustStore, error) {
	if len(keys) == 0 {
		return nil, errors.New("qurlv2: trust store must contain at least one issuer key")
	}
	copied := make(map[string]*ecdsa.PublicKey, len(keys))
	for kid, pub := range keys {
		if kid == "" {
			return nil, errors.New("qurlv2: trust store kid must not be empty")
		}
		if err := validateP256PublicKey(pub); err != nil {
			return nil, fmt.Errorf("qurlv2: trust store key %q: %w", kid, err)
		}
		copied[kid] = pub
	}
...
func ParseP256PublicKeyDER(der []byte) (*ecdsa.PublicKey, error) {
	if len(der) == 0 {
		return nil, errors.New("qurlv2: empty public-key DER")
	}
	parsed, err := x509.ParsePKIXPublicKey(der)
	if err != nil {
		return nil, fmt.Errorf("qurlv2: parse SPKI public key: %w", err)
	}
	pub, ok := parsed.(*ecdsa.PublicKey)
	if !ok {
		return nil, fmt.Errorf("qurlv2: issuer key is %T, want *ecdsa.PublicKey", parsed)
	}
	if err := validateP256PublicKey(pub); err != nil {
		return nil, err
	}
...
// ecdsa.PublicKey coordinate fields (deprecated and unsafe to inspect per
// Go 1.26). Both construction paths also guarantee validity independently
// (x509.ParsePKIXPublicKey rejects off-curve DER), so this is defense in depth.
func validateP256PublicKey(pub *ecdsa.PublicKey) error {
	if pub == nil {
		return errors.New("qurlv2: nil public key")
	}
	if pub.Curve != curve {
		return errors.New("qurlv2: issuer key is not on the P-256 curve")
	}
	...
	if _, err := pub.ECDH(); err != nil {
		return fmt.Errorf("qurlv2: issuer public key is not a valid P-256 point: %w", err)
	}
	return nil
}
