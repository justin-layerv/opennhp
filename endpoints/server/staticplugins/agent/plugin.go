...
// RequestOTP remains only because the shared plugin interface requires it.
// Native Connector OTP is handled by the assigned-cell composition before
// plugin dispatch. Fail closed so the retired HTTP registrar cannot reappear as
...
// RegisterAgent is the corresponding fail-closed interface stub. Native REG is
// completed by the assigned-cell composition before plugin dispatch.
