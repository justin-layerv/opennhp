...
// invalid, expired, consumed, or denied by policy. Matches the access-error
// panel of the qurl.link shell (terraform/modules/qurl-link/frontend/
// index.html) and the website's wordmark-branded palette. Generic message —
// no token state leaked.
//
...
:root{--ink:#0a0a0a;--paper:#f5f5f0;--muted:#c9c9c4;--quiet:#939390;--accent-cyan:#07dcef;--accent-cyan-rgb:7,220,239;--threat-red:#ef4444}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:radial-gradient(700px 500px at 22% 12%,rgba(var(--accent-cyan-rgb),0.06),transparent 70%),var(--ink);color:var(--paper);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:1.25rem}
.container{width:100%;max-width:420px;padding:1.5rem 1.4rem;border:1px solid rgba(255,255,245,0.16);border-radius:8px;background:rgba(22,22,26,0.94);box-shadow:0 30px 90px rgba(0,0,0,0.42);text-align:center}
.wordmark{margin:0.35rem 0 1.5rem}
.wordmark svg{width:150px;height:auto}
h2{font-size:1.35rem;font-weight:700;letter-spacing:-0.02em;color:var(--threat-red);margin-bottom:0.55rem}
p{font-size:.9375rem;line-height:1.6;color:var(--muted);margin-bottom:1.5rem}
.footer{font-size:.75rem}
.footer a{color:var(--quiet);text-decoration:none}
.footer a:hover,.footer a:focus-visible{color:var(--accent-cyan)}
</style>
</head>
<body>
<div class="container">
