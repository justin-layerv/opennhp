...
// + identity supersede the lower in place while preserving one pending entry
// per targeted slot per identity.
//
...
func (s *UdpServer) trackFanout(conns []*ACConn, scope, scopeKey string, epoch int64, eventId string) {
	...
	for _, conn := range conns {
		...
		if strings.TrimSpace(conn.ACId) == "" {
			// Match resolveACIdentityFromPubkey's empty-ACId unresolved path: no
			// pending entry is created for malformed live registry state.
			if s.metrics != nil {
				s.metrics.IncrCounter(MetricRevocationUntrackable)
			}
			log.Error("server-ac(<empty>)[RevocationRetry] cannot track fanout: ACConn missing ACId (scope=%q key=%q epoch=%d eventId=%q); fired %s — a revocation could not be keyed for proof tracking",
				scope, scopeKey, epoch, eventId, MetricRevocationUntrackable)
			continue
		}
		acPubkey, ok := acConnPubkey(conn)
		if !ok {
			// Defense-in-depth: production fanout normally dereferences
			// conn.ACPeer before trackFanout runs, so this branch should only be
			// reachable from malformed tests or future callers that bypass fanout.
			if s.metrics != nil {
				s.metrics.IncrCounter(MetricRevocationUntrackable)
			}
			log.Error("server-ac(%s)[RevocationRetry] cannot track fanout: ACConn missing pubkey (scope=%q key=%q epoch=%d eventId=%q); fired %s — a revocation could not be keyed for proof tracking",
				conn.ACId, scope, scopeKey, epoch, eventId, MetricRevocationUntrackable)
			continue
		}
...
func (s *UdpServer) redeliverPendingRevocation(item dueItem) {
	conn, ok := s.liveACConnForPending(item.key)
	if !ok {
	...
	sent, _ := s.fanoutRevocation([]*ACConn{conn}, revBytes)
	if sent == 0 {
		return
	}
...
// liveACConnForPending snapshots the live AC connection that matches a pending
// entry's exact AC slot identity under acConnectionMapMutex.RLock. Under the
// normal same-pubkey replacement invariant this is at most one connection; if a
// transient duplicate same-pubkey entry appears, redelivery to the first match
// is enough because NHP_REV apply is epoch-idempotent and a duplicate send would
// be redundant.
func (s *UdpServer) liveACConnForPending(key pendingRevocationKey) (*ACConn, bool) {
