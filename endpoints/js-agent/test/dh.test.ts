...
import { x25519PublicKey, x25519SharedSecret, X25519_KEY_SIZE } from "../src/crypto/dh";
...
const ALICE_PRIV = fromHex("77076d0a7318a57d3c16c17251b26645df4c2f87ebc0992ab177fba51db92c2a");
const ALICE_PUB = "8520f0098930a754748b7ddcb43ef75a0dbf3a0d26381af4eba4a98eaa9b4e6a";
const BOB_PRIV = fromHex("5dab087e624a8a4b79e17f8b83800ee66f3bb1292618b6fd1c2f8b27ff88e0eb");
const BOB_PUB = fromHex("de9edb7d7b7dc1b4d35b61c2ece435373f8343c85b78674dadfc7e146f882b4f");
...
describe("X25519 DH", () => {
  ...
  it("rejects a wrong-length peer key", () => {
    expect(() => x25519SharedSecret(ALICE_PRIV, new Uint8Array(X25519_KEY_SIZE - 1))).toThrow();
  });
  ...
  it("rejects a low-order peer key (all-zeros) — fail-loud, matching Go", () => {
    ...
    expect(() => x25519SharedSecret(ALICE_PRIV, new Uint8Array(X25519_KEY_SIZE))).toThrow();
  });

  it("rejects a wrong-length private key (both entry points)", () => {
    expect(() => x25519PublicKey(new Uint8Array(X25519_KEY_SIZE - 1))).toThrow();
    expect(() => x25519SharedSecret(new Uint8Array(X25519_KEY_SIZE - 1), BOB_PUB)).toThrow();
  });
});
