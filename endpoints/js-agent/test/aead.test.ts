...
import { aeadSeal, aeadOpen, AEAD_KEY_SIZE, AEAD_NONCE_SIZE, AEAD_TAG_SIZE } from "../src/crypto/aead";
...
describe("AES-256-GCM AEAD", () => {
  it("matches a known AES-256-GCM tag (key=0, nonce=0, empty plaintext/aad)", () => {
    ...
    const ct = aeadSeal(new Uint8Array(32), new Uint8Array(12), new Uint8Array(0), new Uint8Array(0));
  ...
  it("rejects a wrong-size key", () => {
    expect(() => aeadSeal(new Uint8Array(AEAD_KEY_SIZE - 16), NONCE, utf8("x"), new Uint8Array(0))).toThrow();
  });

  it("rejects a wrong-size nonce", () => {
    expect(() => aeadSeal(KEY, new Uint8Array(AEAD_NONCE_SIZE - 1), utf8("x"), new Uint8Array(0))).toThrow();
  });

  it("rejects a ciphertext shorter than the tag", () => {
    expect(() => aeadOpen(KEY, NONCE, new Uint8Array(AEAD_TAG_SIZE - 1), new Uint8Array(0))).toThrow();
  });

  it("exposes the NHP suite sizes", () => {
    expect([AEAD_KEY_SIZE, AEAD_NONCE_SIZE, AEAD_TAG_SIZE]).toEqual([32, 12, 16]);
  });
});
