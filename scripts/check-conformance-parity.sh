...
if [ -z "${NPM_VECTOR:-}" ]; then
  NPM_VERSION="$(node -p '(o=>o.devDependencies?.["'"${NPM_PACKAGE}"'"]??o.dependencies?.["'"${NPM_PACKAGE}"'"]??"")(JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")))' "${JS_AGENT_DIR}/package.json" 2>/dev/null \
    || true)"
