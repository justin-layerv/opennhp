...
GO_LINUX_ARM64_SHA256=fe4789e92b1f33358680864bbe8704289e7bb5fc207d80623c308935bd696d49
GO_LINUX_ARMV6L_SHA256=6dae9edab81c13bccf962dec15f1fd2ec26c14a6821b4d2c92dab4130c289d7a
...
test_workflow_drift_fails() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak "s/GO_VERSION: '1.26.5'/GO_VERSION: '1.26.6'/" "$tmp/.github/workflows/codeql.yml"
...
test_two_part_go_directive_drift_fails() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak 's/go 1.26.5/go 1.27/' "$tmp/endpoints/go.mod"
  assert_failure "$name" "$tmp" "endpoints/go.mod: expected Go 1.26.5, got 1.27"
}
...
test_source_go_directive_requires_patch() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak 's/go 1.26.5/go 1.26/' "$tmp/nhp/go.mod"
...
test_go_directive_comments_and_whitespace_pass() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak 's/go 1.26.5/go 1.26.5   \/\/ CI lockstep/' "$tmp/endpoints/go.mod"
  printf '\ntoolchain go1.26.5   // CI lockstep\n' >> "$tmp/tests/local/go.mod"
  assert_success "$name" "$tmp"
}
...
test_toolchain_directive_drift_fails() {
  ...
  write_good_fixture "$tmp"
  printf '\ntoolchain go1.26.6\n' >> "$tmp/nhp/go.mod"
  assert_failure "$name" "$tmp" "nhp/go.mod toolchain directive: expected Go 1.26.5, got 1.26.6"
}
...
test_unpinned_test_image_fails() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak "s|GO_TEST_IMAGE: 'golang:1.26.5-bookworm@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'|GO_TEST_IMAGE: 'golang:1.26.5-bookworm'|" "$tmp/.github/workflows/build-and-push.yml"
...
test_docker_from_drift_fails() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak "s/golang:1.26.5-bookworm/golang:1.26.6-bookworm/" "$tmp/docker/Dockerfile.server"
...
test_docker_from_variant_fails_with_variant_error() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak "s/golang:1.26.5-bookworm/golang:1.26.5-alpine/" "$tmp/docker/Dockerfile.ac"
...
FROM golang:1.26.5-bookworm@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa AS second-builder
EOF
  assert_success "$name" "$tmp"
}
...
test_second_dockerfile_golang_from_digest_drift_fails() {
  ...
  write_good_fixture "$tmp"
  cat >> "$tmp/docker/Dockerfile.server" <<EOF
FROM golang:1.26.5-bookworm@sha256:9999999999999999999999999999999999999999999999999999999999999999 AS second-builder
EOF
...
FROM golang:1.26.5-bookworm@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa AS builder
EOF
...
test_dev_dockerfile_quoted_args_pass() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak "s/ARG GO_VERSION=1.26.5/ARG GO_VERSION='1.26.5'/" "$tmp/docker/Dockerfile.app"
...
test_dev_dockerfile_stale_matching_checksums_fail() {
  ...
  assert_failure "$name" "$tmp" "expected GO_LINUX_ARM64_SHA256 checksum $GO_LINUX_ARM64_SHA256 for Go 1.26.5"
}
...
test_checksum_table_version_drift_fails() {
  ...
  trap 'rm -rf "$tmp"' RETURN
  write_good_fixture "$tmp"
  sed -i.bak 's/go 1.26.5/go 1.26.6/' "$tmp/nhp/go.mod"
  assert_failure "$name" "$tmp" "Go tarball checksum table: expected checksums for Go 1.26.6, table is for Go 1.26.5"
}
