...
#   no_needs_app_image_build — drop classifier job from notify.needs (C1)
#   no_env_rebuilt    — drop the APP_REBUILT env line                 (C2)
#   no_needs_setup    — drop `- setup` from notify.needs              (C4)
#   no_env_image_tag  — drop the IMAGE_TAG env line                   (C5)
#   no_rebuilt_cond   — rebuilt branch tests a different var          (C6)
#   no_rebuilt_sha    — rebuilt branch drops ${IMAGE_TAG:0:7}         (C7)
#   no_unchanged_cond — unchanged branch tests a different var        (C8)
#   no_unchanged_str  — unchanged branch sets a non-canonical string  (C9)
#   no_unknown_str    — fallback branch sets a non-canonical string   (C10)
#   no_outer_gate     — replace SANDBOX_DEPLOYED gate with `if true`  (C11)
#   no_render_gate    — render the block WITHOUT the `-n` gate        (C12 anchor)
#   no_render_block   — keep the gate but drop the *App image:* block (C12 target)
...
make_wf() {
  ...
  local f="$TMP/wf_${variant}_$RANDOM.yml"
  {
    ...
    if [ "$variant" = "no_notify" ]; then
      ...
      echo "    needs:"
      echo "      - build"
      [ "$variant" != "no_needs_app_image_build" ] && echo "      - app-image-build-required"
      ...
      if [ "$variant" != "no_env_rebuilt" ]; then
        echo "          APP_REBUILT: \${{ needs.app-image-build-required.outputs.app_image_build_required }}"
      fi
...
# C1: app-image-build-required dropped from notify.needs -> exit 1
assert_exit "bad C1: 'app-image-build-required' missing from notify.needs" \
  "$(make_wf no_needs_app_image_build)" 1

# C2: APP_REBUILT env not forwarded -> exit 1
assert_exit "bad C2: APP_REBUILT env not forwarded" \
  "$(make_wf no_env_rebuilt)" 1

# C4: setup dropped from notify.needs -> exit 1
assert_exit "bad C4: 'setup' missing from notify.needs" \
  "$(make_wf no_needs_setup)" 1

# C5: IMAGE_TAG env not forwarded -> exit 1
assert_exit "bad C5: IMAGE_TAG env not forwarded from needs.setup.outputs.image_tag" \
  "$(make_wf no_env_image_tag)" 1

# C6: rebuilt branch tests a different var -> exit 1
assert_exit "bad C6: rebuilt branch no longer keys on APP_REBUILT==true" \
  "$(make_wf no_rebuilt_cond)" 1

# C7: rebuilt branch dropped the short sha -> exit 1
assert_exit "bad C7: rebuilt branch dropped \${IMAGE_TAG:0:7}" \
  "$(make_wf no_rebuilt_sha)" 1

# C8: unchanged branch tests a different var -> exit 1
assert_exit "bad C8: unchanged branch no longer keys on APP_REBUILT==false" \
  "$(make_wf no_unchanged_cond)" 1

# C9: unchanged branch's string gutted -> exit 1
assert_exit "bad C9: unchanged branch no longer sets the 'unchanged …' string" \
  "$(make_wf no_unchanged_str)" 1

# C10: fallback branch's string gutted -> exit 1
assert_exit "bad C10: fallback branch no longer sets 'rebuild status unknown …'" \
  "$(make_wf no_unknown_str)" 1

# C11: SANDBOX_DEPLOYED gate replaced with `if true` -> exit 1
assert_exit "bad C11: App-image computation not gated on SANDBOX_DEPLOYED==true" \
  "$(make_wf no_outer_gate)" 1

# C12 anchor: render gate removed (block renders unconditionally) -> exit 1
assert_exit "bad C12: *App image:* block detached from its -n \$APP_IMAGE gate" \
  "$(make_wf no_render_gate)" 1

# C12 target: gate present but *App image:* block gone -> exit 1
assert_exit "bad C12: -n \$APP_IMAGE gate present but *App image:* block missing" \
