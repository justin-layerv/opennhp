...
#
# Copies the three real source-of-truth files into a tempdir that mimics the
...
#     Makefile xdp path, Makefile tc path, Dockerfile guard path) is DETECTED
#     (exit non-zero) — this proves the lint is non-vacuous: it actually fails
#     when a site drifts, not just when nothing changed,
...
# Build a fixture repo: copy the three real files into the same relative paths
# under a tempdir. The script reads only those three, so nothing else is needed.
_make_fixture() {
  local dir="$1"
  mkdir -p "$dir/endpoints/ac/ebpf" "$dir/docker"
