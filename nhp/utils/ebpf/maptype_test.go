...
// TestXdpSource_AllowRuleMapsAreHash_ConnTrackIsLru is the declaration-level
// regression guard for the #2163 fix. It parses the XDP C source and asserts
// the BPF_MAP_TYPE of each named map. This is complementary to the kernel
// behavioral test: the behavioral test proves the SEMANTICS (HASH rejects on
// full, LRU evicts); this test proves the DECLARATIONS in source haven't
// regressed (someone flipping spp back to LRU_HASH fails CI here even on a
// machine that can't run BPF).
//
// Per-map is essential: a naive "no LRU_HASH anywhere" grep would wrongly flag
// conn_track, which legitimately MUST stay LRU_HASH (it is a derived cache, not
// an authoritative allow-rule — see the .c comment and
// SESSION_ENFORCEMENT_ARCHITECTURE.md).
//
// NOTE: this intentionally checks the .c SOURCE, not the compiled
// endpoints/ac/main/etc/nhp_ebpf_xdp.o — that checked-in object is stale
// (predates the current source: missing spp/src_port, port_list/protocol_port
// at max_entries=10000) and is regenerated by a sibling slice (E1) from this
// source. Asserting against the stale object would be both flaky and dishonest.
func TestXdpSource_AllowRuleMapsAreHash_ConnTrackIsLru(t *testing.T) {
	src := readXdpSource(t)
	...
	want := map[string]string{
		...
		// per-flow derived cache — deliberately LRU_HASH
		"conn_track": "BPF_MAP_TYPE_LRU_HASH",
	}

	for name, wantType := range want {
		got := mapTypeForName(t, src, name)
		if got != wantType {
			t.Errorf("map %q declared as %s, want %s (#2163: authoritative allow-rule maps must be HASH so a full map fails the new admission closed instead of silently evicting; conn_track stays LRU as a derived cache)", name, got, wantType)
		}
	}
}

// readXdpSource loads nhp/ebpf/xdp/nhp_ebpf_xdp.c via a path resolved relative
// to THIS test file (runtime.Caller), so it is independent of the working
// directory the test runner uses.
func readXdpSource(t *testing.T) string {
	t.Helper()
	_, thisFile, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("runtime.Caller failed; cannot locate test file to resolve the .c path")
	}
	// this file: nhp/utils/ebpf/maptype_test.go -> repo nhp module root is ../../..
	srcPath := filepath.Join(filepath.Dir(thisFile), "..", "..", "ebpf", "xdp", "nhp_ebpf_xdp.c")
