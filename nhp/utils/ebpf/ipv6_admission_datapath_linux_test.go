...
const (
	...
	ipprotoHopByHop   = 0   // IPv6 Hop-by-Hop Options extension header
	ipprotoRouting    = 43  // IPv6 Routing extension header
	ipprotoFragment   = 44  // IPv6 Fragment extension header
	ipprotoESP        = 50  // IPv6 Encapsulating Security Payload
	ipprotoAH         = 51  // IPv6 Authentication Header
	ipprotoNoNext     = 59  // IPv6 No Next Header
	ipprotoDstOpts    = 60  // IPv6 Destination Options extension header
...
func seedWhitelistValue(t *testing.T, valueSize uint32) []byte {
	t.Helper()
	...
	if int(valueSize) != WhitelistValueSize {
		t.Fatalf("allow-rule map value size %d != Go whitelistValue size %d (allowed@0 + expire_time@%d) — the kernel sdwhitelist_value/whitelist_value/icmpwhitelist_value layout drifted from the Go struct; reconcile before trusting the offset write",
...
func appendTCPv6Header(pkt []byte, srcPort, dstPort uint16) []byte {
	const tcpLen = 20
...
// parses the 40-byte fixed IPv6 header, resolves any supported extension-header
// chain, then reads the TCP header for sport/dport.
...
func craftTCPv6Packet(t *testing.T, srcIP6, dstIP6 [16]byte, srcPort, dstPort uint16) []byte {
	...
	pkt = appendTCPv6Header(pkt, srcPort, dstPort)

	return pkt
}

// craftTCPv6PacketWithExtHeaders builds an IPv6 TCP packet whose fixed header
// points at one or more 8-byte extension headers before TCP. The extHdrs slice
// is the wire-order list of extension-header protocol numbers; each synthetic
// extension header has hdrlen=0 (8 bytes total) and points to the next extension
// header, with the final one pointing at TCP.
func craftTCPv6PacketWithExtHeaders(t *testing.T, srcIP6, dstIP6 [16]byte, srcPort, dstPort uint16, extHdrs ...uint8) []byte {
	t.Helper()
	if len(extHdrs) == 0 {
		return craftTCPv6Packet(t, srcIP6, dstIP6, srcPort, dstPort)
	}
	...
	payloadLen := uint16(len(extHdrs)*extLen + tcpLen)
	pkt := ethIPv6Header(srcIP6, dstIP6, extHdrs[0], payloadLen)
	for i := range extHdrs {
		next := uint8(ipprotoTCP)
		if i+1 < len(extHdrs) {
			next = extHdrs[i+1]
		}
		ext := make([]byte, extLen)
		ext[0] = next // next header
...
func craftTCPv6PacketWithRoutingHeader(t *testing.T, srcIP6, dstIP6 [16]byte, srcPort, dstPort uint16, hdrExtLen, segmentsLeft uint8) []byte {
	t.Helper()

	const tcpLen = 20
	routingLen := (int(hdrExtLen) + 1) * 8
	pkt := ethIPv6Header(srcIP6, dstIP6, ipprotoRouting, uint16(routingLen+tcpLen))

	routing := make([]byte, routingLen)
	routing[0] = ipprotoTCP
	routing[1] = hdrExtLen
	routing[2] = 0 // routing type; kernel owns semantic validation after XDP_PASS
	routing[3] = segmentsLeft
	pkt = append(pkt, routing...)

	return appendTCPv6Header(pkt, srcPort, dstPort)
}

// craftTCPv6PacketWithAH builds a packet with a single Authentication Header in
// front of TCP. AH is split out from craftTCPv6PacketWithExtHeaders because its
// length field uses 4-byte units with a +2 bias, unlike the option-style
// extension headers' 8-byte unit formula.
func craftTCPv6PacketWithAH(t *testing.T, srcIP6, dstIP6 [16]byte, srcPort, dstPort uint16, ahPayloadLen uint8) []byte {
	t.Helper()

	const tcpLen = 20
	ahLen := (int(ahPayloadLen) + 2) * 4
	pkt := ethIPv6Header(srcIP6, dstIP6, ipprotoAH, uint16(ahLen+tcpLen))
...
func craftTCPv6PacketWithHopByHopAH(t *testing.T, srcIP6, dstIP6 [16]byte, srcPort, dstPort uint16, ahPayloadLen uint8) []byte {
	t.Helper()
	...
	ahLen := (int(ahPayloadLen) + 2) * 4
	pkt := ethIPv6Header(srcIP6, dstIP6, ipprotoHopByHop, uint16(hopByHopLen+ahLen+tcpLen))

	hopByHop := make([]byte, hopByHopLen)
	hopByHop[0] = ipprotoAH
	hopByHop[1] = 0
...
func craftIPv6PacketWithNextHeader(srcIP6, dstIP6 [16]byte, nextHdr uint8, payload []byte) []byte {
	pkt := ethIPv6Header(srcIP6, dstIP6, nextHdr, uint16(len(payload)))
	return append(pkt, payload...)
}

func craftIPv6PacketWithTruncatedExtHeader(t *testing.T, srcIP6, dstIP6 [16]byte) []byte {
	t.Helper()
	ext := make([]byte, 8)
	ext[0] = ipprotoTCP
	ext[1] = 1 // claims 16 bytes, but only 8 bytes are present
	return craftIPv6PacketWithNextHeader(srcIP6, dstIP6, ipprotoDstOpts, ext)
}

func craftIPv6PacketWithTruncatedAH(t *testing.T, srcIP6, dstIP6 [16]byte) []byte {
	t.Helper()
	ah := make([]byte, 8)
	ah[0] = ipprotoTCP
	ah[1] = 2 // claims 16 bytes by AH's (payload_len + 2) * 4 formula
...
func craftUDPv6Packet(t *testing.T, srcIP6, dstIP6 [16]byte, srcPort, dstPort uint16) []byte {
	...
	// UDP checksum.
	pkt = appendUDPv6Header(pkt, srcPort, dstPort)

	return pkt
}

func craftUDPv6PacketWithExtHeaders(t *testing.T, srcIP6, dstIP6 [16]byte, srcPort, dstPort uint16, extHdrs ...uint8) []byte {
	t.Helper()
	if len(extHdrs) == 0 {
		return craftUDPv6Packet(t, srcIP6, dstIP6, srcPort, dstPort)
	}
	...
	payloadLen := uint16(len(extHdrs)*extLen + udpLen)
	pkt := ethIPv6Header(srcIP6, dstIP6, extHdrs[0], payloadLen)
	for i := range extHdrs {
		next := uint8(ipprotoUDP)
		if i+1 < len(extHdrs) {
			next = extHdrs[i+1]
		}
		ext := make([]byte, extLen)
...
func craftICMPv6EchoRequestWithExtHeaders(t *testing.T, srcIP6, dstIP6 [16]byte, extHdrs ...uint8) []byte {
	t.Helper()
	if len(extHdrs) == 0 {
		return craftICMPv6EchoRequest(t, srcIP6, dstIP6)
	}
	...
	payloadLen := uint16(len(extHdrs)*extLen + icmp6Len)
	pkt := ethIPv6Header(srcIP6, dstIP6, extHdrs[0], payloadLen)
	for i := range extHdrs {
		next := uint8(ipprotoICMPv6)
		if i+1 < len(extHdrs) {
			next = extHdrs[i+1]
		}
		ext := make([]byte, extLen)
...
// pick the one they need via requireMap.
func loadXDPCollectionV6(t *testing.T) (*ebpf.Program, *ebpf.Collection, func()) {
	t.Helper()
	return loadXDPCollectionInner(t)
}

// requireMap fetches a named map handle from the loaded collection, failing
...
// map was never integrated into the object — so this Fatals regardless of
// the NHP_REQUIRE_BPF_TESTS guard, mirroring the program/map nil-checks in
// loadXDPCollectionInner and loadXDPCollection.
func requireMap(t *testing.T, coll *ebpf.Collection, name string) *ebpf.Map {
	t.Helper()
	m := coll.Maps[name]
	if m == nil {
		t.Fatalf("map %q not found in XDP object (symbol renamed in nhp_ebpf_xdp.c, or not integrated into the object?)", name)
	}
	return m
}
...
// TestIPv6AdmissionDatapathExtensionHeaders proves the v6 parser now performs a
// bounded extension-header walk before the ordinary TCP allow-rule cascade.
// Before #2833, any packet whose fixed IPv6 header pointed at Hop-by-Hop or
// Destination Options dropped before the allow-rule lookup, even when the chain
// ended in TCP and the source/destination had a valid sdwhitelist_v6 rule.
func TestIPv6AdmissionDatapathExtensionHeaders(t *testing.T) {
	prog, coll, cleanup := loadXDPCollectionV6(t)
	if cleanup != nil {
		defer cleanup()
	}
	...
	allowPkt := craftTCPv6PacketWithExtHeaders(t, allowedSrc, dstIP, srcPort, dstPort, ipprotoHopByHop, ipprotoDstOpts)
	denyPkt := craftTCPv6PacketWithExtHeaders(t, deniedSrc, dstIP, srcPort, dstPort, ipprotoHopByHop, ipprotoDstOpts)
	assertDifferOnlyInV6SrcAddr(t, allowPkt, denyPkt)

	if got := runVerdict(t, prog, allowPkt); got != xdpPass {
		t.Fatalf("extension-header PASS case: verdict = %d (%s), want XDP_PASS(%d) — a supported Hop-by-Hop→DestinationOptions→TCP chain matching sdwhitelist_v6 must resolve to TCP and be admitted",
			got, xdpName(got), xdpPass)
	}

	if got := runVerdict(t, prog, denyPkt); got != xdpDrop {
		t.Fatalf("extension-header DROP case: verdict = %d (%s), want XDP_DROP(%d) — resolving extension headers must not bypass the ordinary v6 allow-rule cascade",
			got, xdpName(got), xdpDrop)
	}
}

// TestIPv6AdmissionDatapathMaxExtensionHeaders proves the named cap means what
// it says: exactly IPV6_EXT_MAX_HEADERS option-style headers can still resolve
// to TCP and enter the allow-rule cascade.
func TestIPv6AdmissionDatapathMaxExtensionHeaders(t *testing.T) {
	prog, coll, cleanup := loadXDPCollectionV6(t)
	if cleanup != nil {
		defer cleanup()
	}
	...
	pkt := craftTCPv6PacketWithExtHeaders(t, allowedSrc, dstIP, 51000, 443,
		ipprotoDstOpts, ipprotoDstOpts, ipprotoDstOpts,
		ipprotoDstOpts, ipprotoDstOpts, ipprotoDstOpts)

	if got := runVerdict(t, prog, pkt); got != xdpPass {
		t.Fatalf("max extension-header chain: verdict = %d (%s), want XDP_PASS(%d) — exactly six extension headers must resolve to TCP and use the normal allow-rule cascade",
			got, xdpName(got), xdpPass)
	}
}

// TestIPv6AdmissionDatapathExtensionHeaderLimitFailClosed fences the bounded
// walk: a chain longer than the verifier-friendly cap stays fail-closed even if
// a later header would eventually point at TCP and the source has an allow-rule.
func TestIPv6AdmissionDatapathExtensionHeaderLimitFailClosed(t *testing.T) {
	prog, coll, cleanup := loadXDPCollectionV6(t)
	if cleanup != nil {
		defer cleanup()
	}
	...
	pkt := craftTCPv6PacketWithExtHeaders(t, allowedSrc, dstIP, 51000, 443,
		ipprotoDstOpts, ipprotoDstOpts, ipprotoDstOpts, ipprotoDstOpts,
		ipprotoDstOpts, ipprotoDstOpts, ipprotoDstOpts)

	if got := runVerdict(t, prog, pkt); got != xdpDrop {
		t.Fatalf("overlong extension-header chain: verdict = %d (%s), want XDP_DROP(%d) — chains beyond the bounded parser cap must fail closed, not eventually admit",
			got, xdpName(got), xdpDrop)
	}
}

// TestIPv6AdmissionDatapathUnsupportedExtensionHeadersFailClosed locks the
// parser's explicit deny exits: unsupported terminal next-header values and a
// truncated option header must drop even when the packet's src/dst has a valid
// allow-rule.
func TestIPv6AdmissionDatapathUnsupportedExtensionHeadersFailClosed(t *testing.T) {
	prog, coll, cleanup := loadXDPCollectionV6(t)
	if cleanup != nil {
		defer cleanup()
	}
	...
	cases := []struct {
		...
		{
			name: "ESP",
			pkt:  craftIPv6PacketWithNextHeader(allowedSrc, dstIP, ipprotoESP, nil),
		},
		{
			name: "NoNextHeader",
			pkt:  craftIPv6PacketWithNextHeader(allowedSrc, dstIP, ipprotoNoNext, nil),
		},
		{
			name: "UnknownNextHeader",
			pkt:  craftIPv6PacketWithNextHeader(allowedSrc, dstIP, ipprotoUnknown, nil),
		},
		{
			name: "TruncatedDestinationOptions",
			pkt:  craftIPv6PacketWithTruncatedExtHeader(t, allowedSrc, dstIP),
		},
		{
			name: "TruncatedAH",
			pkt:  craftIPv6PacketWithTruncatedAH(t, allowedSrc, dstIP),
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := runVerdict(t, prog, tc.pkt); got != xdpDrop {
				t.Fatalf("%s unsupported/truncated extension path: verdict = %d (%s), want XDP_DROP(%d) — parser must fail closed before allow-rule admission",
					tc.name, got, xdpName(got), xdpDrop)
			}
		})
	}
}

// TestIPv6AdmissionDatapathAHRoutingMobilityHeaders covers the supported
// extension-header branches not exercised by the Hop-by-Hop/Destination Options
// test. AH is the important one: it uses a different length formula, so this
// drives a non-zero AH payload-len field through the verifier-accepted program.
func TestIPv6AdmissionDatapathAHRoutingMobilityHeaders(t *testing.T) {
	prog, coll, cleanup := loadXDPCollectionV6(t)
	if cleanup != nil {
		defer cleanup()
	}
	...
	cases := []struct {
		...
		{
			name: "AH_min_len",
			pkt:  craftTCPv6PacketWithAH(t, allowedSrc, dstIP, 50999, 443, 0),
		},
		{
			name: "AH_nonzero_len",
			pkt:  craftTCPv6PacketWithAH(t, allowedSrc, dstIP, 51000, 443, 2),
		},
		{
			name: "HopByHop_AH_mixed",
			pkt:  craftTCPv6PacketWithHopByHopAH(t, allowedSrc, dstIP, 51003, 443, 2),
		},
		{
			name: "Routing",
			pkt:  craftTCPv6PacketWithExtHeaders(t, allowedSrc, dstIP, 51001, 443, ipprotoRouting),
		},
		{
			name: "Routing_nonzero_len_segments_left",
			pkt:  craftTCPv6PacketWithRoutingHeader(t, allowedSrc, dstIP, 51004, 443, 1, 1),
		},
		{
			name: "Mobility",
			pkt:  craftTCPv6PacketWithExtHeaders(t, allowedSrc, dstIP, 51002, 443, ipprotoMobility),
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := runVerdict(t, prog, tc.pkt); got != xdpPass {
				t.Fatalf("%s extension-header PASS case: verdict = %d (%s), want XDP_PASS(%d) — supported extension header must resolve to TCP and hit sdwhitelist_v6",
					tc.name, got, xdpName(got), xdpPass)
			}
		})
	}
}
...
func TestIPv6AdmissionDatapathUDP(t *testing.T) {
	...
	wlVal := seedWhitelistValue(t, sppMap.ValueSize())
	...
	allowExtPkt := craftUDPv6PacketWithExtHeaders(t, allowedSrc, dstIP, srcPort+1, dstPort, ipprotoDstOpts)
	if got := runVerdict(t, prog, allowExtPkt); got != xdpPass {
		t.Fatalf("UDP extension-header PASS case: verdict = %d (%s), want XDP_PASS(%d) — DestinationOptions→UDP must resolve to the UDP branch and honor the seeded spp_v6 rule",
			got, xdpName(got), xdpPass)
	}
}
...
func TestIPv6AdmissionDatapathICMPv6(t *testing.T) {
	...
	icmpMap := requireMap(t, coll, icmpWlV6MapName)
	...
	// {allowed, expire_time} layout as sdwhitelist_value, so seedWhitelistValue
	// produces a correctly-sized never-expire value.
	icmpKey := (&srcDestKeyV6{SrcIP: allowedSrc, DstIP: dstIP}).ToSdKeyV6()
	icmpVal := seedWhitelistValue(t, icmpMap.ValueSize())
	...
	allowExtPkt := craftICMPv6EchoRequestWithExtHeaders(t, allowedSrc, dstIP, ipprotoHopByHop)
	denyExtPkt := craftICMPv6EchoRequestWithExtHeaders(t, deniedSrc, dstIP, ipprotoHopByHop)
	assertDifferOnlyInV6SrcAddr(t, allowExtPkt, denyExtPkt)
	if got := runVerdict(t, prog, allowExtPkt); got != xdpPass {
		t.Fatalf("ICMPv6 extension-header PASS case: verdict = %d (%s), want XDP_PASS(%d) — Hop-by-Hop→ICMPv6 must resolve to the ICMPv6 branch and honor the seeded icmp_wl_v6 rule",
			got, xdpName(got), xdpPass)
	}
	if got := runVerdict(t, prog, denyExtPkt); got != xdpDrop {
		t.Fatalf("ICMPv6 extension-header DROP case: verdict = %d (%s), want XDP_DROP(%d) — unmatched Hop-by-Hop→ICMPv6 must resolve to the ICMPv6 branch and still fail closed on icmp_wl_v6 miss",
			got, xdpName(got), xdpDrop)
	}
}
...
func TestIPv6AdmissionDatapathSrcPort(t *testing.T) {
	...
	spMap := requireMap(t, coll, srcPortV6MapName)
	...
	// same layout as sdwhitelist_value -> seedWhitelistValue sizes it correctly).
	spKey := (&srcIPdstPortKeyV6{SrcIP: allowedSrc, DstPort: dstPort}).ToSpKeyV6()
	spVal := seedWhitelistValue(t, spMap.ValueSize())
...
func TestIPv6AdmissionDatapathPortList(t *testing.T) {
	...
	plMap := requireMap(t, coll, portListV6MapName)
