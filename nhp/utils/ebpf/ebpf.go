...
// require the marshaled key to be (a wrong length fails loudly with
...
// ipv4_ct_tuple` is intentionally NOT packed. The struct's largest fields are
// __be32, so C gives it 4-byte alignment and rounds its 14 field bytes up to
// a 16-byte size. The sibling allow-rule key structs (spp, src_port, ...)
// use the fully-spelled `__attribute__((packed))` and keep their exact sizes
// (e.g. spp KeySize=11, NOT rounded to 12), which is how we know 16 is the
// natural-layout ABI for this specific tuple, not clang rounding map keys in
// general.
//
// Historical footgun (#2818): this tuple used to end with a bare
// `} __packed;` token. `__packed` is the Linux `compiler.h` shorthand for
// `__attribute__((packed))`, but that header is not included in the XDP
// translation unit, so the token applied no attribute and emitted a stray ELF
// symbol named `__packed`. The C source now removes that misleading token and
// pins the natural 16-byte ABI with a `_Static_assert`; the committed-object
// contract test also rejects a reintroduced `__packed` symbol. (Verified
// against endpoints/ac/main/etc/nhp_ebpf_xdp.o: conn_track KeySize=16,
// spp=11.)
//
...
//
// Repacking this tuple to 14 bytes would be a breaking change for the
// LIBBPF_PIN_BY_NAME conn_track map and must be coordinated with a pinned-map
// migration plus a lockstep Go serializer change. Until then, 16 is the live
// ABI and this serialization matches it.
//
