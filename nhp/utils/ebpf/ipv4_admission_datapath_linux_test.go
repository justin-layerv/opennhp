...
// surfaced as loadXDPCollectionV6), the named-map fetch (requireMapV6 — generic
...
// (seedWhitelistValueV6) is also reused for the four non-packed-value maps
...
const (
	...
	// in the C source makes the requireMapV6 lookup return a nil map -> hard
...
// (spp/sdwhitelist/src_port/port_list), whose C value structs natural-align to
// 16 bytes and are seeded by seedWhitelistValueV6, the C `protocol_port_value`
...
// Go procoPortValue struct, by contrast, natural-aligns to 16 bytes
// (WhitelistValueSize), so seedWhitelistValueV6 would (correctly) Fatal on the
...
// seedWhitelistValueV6: the kernel checks `val->expire_time < bpf_ktime_get_ns()`
...
// rule). It is the v4 analogue of v6TestAddrs. The addresses are returned as
...
// they actually differ somewhere inside it. This is the v4 analogue of
...
func TestIPv4AdmissionDatapathPortListAllPorts(t *testing.T) {
	...
	plMap := requireMapV6(t, coll, portListV4MapName)
	...
	// non-packed {allowed, expire_time} = 16 bytes -> seedWhitelistValueV6 sizes
	// it correctly).
...
func TestIPv4AdmissionDatapathWhitelist(t *testing.T) {
	...
	sppMap := requireMapV6(t, coll, sppV4MapName)
	...
	// = 16 bytes -> seedWhitelistValueV6 sizes it correctly).
	...
	if err := sppMap.Put(wlKey, seedWhitelistValueV6(t, sppMap.ValueSize())); err != nil {
...
func TestIPv4AdmissionDatapathSdWhitelist(t *testing.T) {
	...
	sdMap := requireMapV6(t, coll, sdWhitelistV4MapName)
	...
	// (sdwhitelist_value is non-packed 16 bytes -> seedWhitelistValueV6).
	sdKey := (&srcDestKey{SrcIP: mustParseIP4(t, allowedSrc), DstIP: mustParseIP4(t, dstIP)}).ToSdKey()
	if err := sdMap.Put(sdKey, seedWhitelistValueV6(t, sdMap.ValueSize())); err != nil {
...
func TestIPv4AdmissionDatapathSrcPort(t *testing.T) {
	...
	spMap := requireMapV6(t, coll, srcPortV4MapName)
	...
	if err := spMap.Put(spKey, seedWhitelistValueV6(t, spMap.ValueSize())); err != nil {
...
func TestIPv4AdmissionDatapathProtocolPort(t *testing.T) {
	...
	ppMap := requireMapV6(t, coll, protocolPortV4MapName)
