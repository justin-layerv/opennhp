...
// Note: This function assumes AES block size (16 bytes). The original implementation
// did not take a blockSize parameter, so we default to 16 for AES compatibility.
// Callers using other block sizes should use PKCS7Unpad directly.
func pkcs5UnPadding(origData []byte) []byte {
	return PKCS7Unpad(origData, 16)
}
